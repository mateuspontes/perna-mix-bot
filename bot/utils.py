...
def extract_groups_from_text(text: str, message: discord.Message) -> List[List[str]]:
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            separators = r'[,;\s\n-]+'
            group_players = [p.strip() for p in re.split(separators, group_content) if p.strip()]
            cleaned_group = []
...
def balance_teams_with_groups(players: List[str], groups: List[List[str]]) -> List[str]:
    ...
    for group in shuffled_groups:
        group_players = [grouped_players_dict[p.lower()] for p in group if p.lower() in grouped_players_dict]
...
def create_team_message(users: List[str], groups: Optional[List[List[str]]] = None) -> str:
    ...
    if len(users) > 10:
        ...
        if groups:
            filtered_groups = []
            for group in groups:
                group_in_players = all(p.lower() in playing_players_set for p in group)
        ...
        if filtered_groups:
            balanced_players = balance_teams_with_groups(playing_players, filtered_groups)
        else:
